...
fn test_windows_service_full_transaction() {
    ...
    policy.expected_state = Some(nogap_core::types::ExpectedState::String("stopped_disabled".to_string()));
    ...
    assert!(!check_result_1.passed, "Initial check should fail (service running/enabled)");

    // Save snapshot before remediation
    snapshot_provider.save_snapshot(&policy.id, "before", "Service running and enabled").unwrap();
    ...
    assert!(!service_manager.is_running("RemoteRegistry").unwrap(), "Service should be stopped");
    assert!(!service_manager.is_enabled("RemoteRegistry").unwrap(), "Service should be disabled");

    // Save snapshot after remediation
    snapshot_provider.save_snapshot(&policy.id, "after", "Service stopped and disabled").unwrap();
    ...
    assert!(check_result_2.passed, "Final check should pass (service stopped/disabled)");

    // Verify snapshots were called exactly twice
    assert_eq!(snapshot_provider.get_count(), 2, "Snapshot provider should be called exactly twice");
}
...
fn test_linux_file_regex_full_transaction() {
    ...
    assert!(!check_result_1.passed, "Initial check should fail (PermitRootLogin yes found)");
    assert!(check_result_1.message.contains("vulnerable pattern"));

    // Save snapshot before remediation
    snapshot_provider.save_snapshot(&policy.id, "before", "PermitRootLogin yes").unwrap();
    ...
    match remediate_result {
        ...
        linux::RemediateResult::Failed(message) => panic!("Remediation should succeed: {}", message),
    }

    // Save snapshot after remediation
    snapshot_provider.save_snapshot(&policy.id, "after", "PermitRootLogin no").unwrap();
    ...
    assert!(check_result_2.passed, "Final check should pass (PermitRootLogin no)");

...
fn test_week2_complete_workflow() {
    ...
    let snapshot_diff = snapshot::compare_snapshots(&conn, id1, id2)
        .expect("Failed to compare snapshots");
    
    println!("\nSnapshot comparison (comparing after_state of snapshots {} and {}):", id1, id2);
    snapshot_diff.display();
    assert!(!snapshot_diff.is_empty(), "Expected differences between snapshots");
    ...
    println!("Queued {} files for signing (parallel threads)", handles.len());
    ...
    for (i, handle) in handles.iter().enumerate() {
        ...
        let mut attempts = 0;
        while matches!(status, auto_signer::SigningStatus::Pending | auto_signer::SigningStatus::InProgress) {

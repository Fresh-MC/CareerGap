...
impl AutoSignerHandle {
    ...
    pub fn wait_for_completion(self) -> SigningStatus {
        ...
        while let Ok(status) = self.receiver.recv() {
            ...
            if matches!(final_status, SigningStatus::Completed { .. } | SigningStatus::Failed(_)) {
                break;
            }
        }
        final_status
...
impl AutoSigner {
    ...
    pub fn watch_directory<P: AsRef<Path>>(
        ...
        println!("👀 Watching {:?} for files matching '{}'", dir_path, pattern);
...
mod tests {
    ...
    fn test_sign_file_async() {
        ...
        for _ in 0..50 {
            ...
            if matches!(status, SigningStatus::Completed { .. } | SigningStatus::Failed(_)) {
                break;
            }
